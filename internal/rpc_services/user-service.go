...
func (u *UserRPC) ResetPassword(userId uint, passwordHash string) error {
	err := u.rpcClient.Call("UserServer.ResetPassword", UserRPCPayload{UserId: userId, PasswordHash: passwordHash}, nil)
	return err
}

func (u *UserRPC) ActivateUser(userId uint) error {
	err := u.rpcClient.Call("UserServer.ActivateUser", userId, nil)
	return err
}

func (u *UserRPC) MatchPassword(email, passwordHash string) (err error) {
	err = u.rpcClient.Call("UserServer.MatchPassword", UserRPCPayload{Email: email, PasswordHash: passwordHash}, nil)
	return
}
